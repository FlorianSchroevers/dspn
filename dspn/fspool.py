...
class FSPool(nn.Module):
    ...
    def forward(self, x, n=None):
        ...
        Variable set sizes should be padded to the maximum set size in the
        batch with 0s.

        n: LongTensor of shape (batch_size).
        This tensor contains the sizes of each set in the batch.
        If not specified, assumes that every set has the same size of
        x.size(2).
        Note that n.max() should never be greater than x.size(2), i.e. the
        specified set size in the
        n tensor must not be greater than the number of elements stored in the
        x tensor.
        ...
        # can call withtout length tensor, uses same length for all sets in
        # the batch
        if n is None:
        ...
        # sort is descending, so put unreasonably low value in places to be
        # masked away
    ...
    def determine_weight(self, sizes):
        ...
        This should be a faster implementation than doing the sum over max
        terms, since we know that most terms in it are 0.
        """
        # share same sequence length within each sample, so copy weight
        # across batch dim
...
def fill_sizes(sizes, x=None):
    """
        ...
        Each set size n is turned into
        [0/(n-1), 1/(n-1), ..., (n-2)/(n-1), 1, 0, 0, ..., 0, 0].
        These are the ratios r at which f is evaluated at.
        The 0s at the end are there for padding to the largest n in the batch.
        If the input set x is passed in, it guarantees that the mask is the
        correct size even when sizes.max()
        is less than x.size(), which can be a case if there is at least one
        padding element in each set in the batch.
    """
    if x is not None:
        max_size = x.size(2)
    else:
    ...
    size_tensor = size_tensor.unsqueeze(0) \
        / (sizes.float() - 1).clamp(min=1).unsqueeze(1)
...
def deterministic_sort(s, tau):
    ...
    scaling = (n + 1 - 2 * (torch.arange(n, device=s.device) + 1)).type(
        torch.float32
    )
...
def cont_sort(x, perm=None, temp=1):
    ...
    Since it assumes a shape of (batch_size, n, 1) while the input x is of
    shape (batch_size, channels, n),
    we can get this to the right shape by merging the first two dimensions.
    If an existing perm is passed in, we compute the "inverse" (transpose of
    perm) and just use that to unsort x.
    """
